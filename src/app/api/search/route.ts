...
import { fallbackProjects, fallbackCourses } from '@/lib/fallback-data'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (projects.length > 0 || courses.length > 0 || lessons.length > 0) {
  ...
  // Hard-coded fallback: search the seed catalog directly.
  const projects = fallbackProjects()
    .filter(p => matches([p.title, p.description, (p.tags || []).join(' ')]))
    .slice(0, 5)
  const courses = fallbackCourses()
    .filter(c => matches([c.title, c.description, c.category]))
    .slice(0, 5)

  return apiSuccess({ projects, courses, lessons: [] })
}
