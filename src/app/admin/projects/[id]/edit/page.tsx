...
export default function EditProjectPage() {
  ...
  const [form, setForm] = useState({
    ...
    category: 'large', subCategory: 'other', techStack: '', status: 'in-progress', featured: false, isPublished: true,
  })
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div><label className="text-sm font-medium mb-1 block">Short Description *</label><Input required maxLength={160} value={form.description} onChange={e => update('description', e.target.value)} /></div>
            ...
            <div><label className="text-sm font-medium mb-1 block">Tech Stack (comma separated)</label><Input value={form.techStack} onChange={e => update('techStack', e.target.value)} placeholder="React, TypeScript, Next.js" /></div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-sm font-medium mb-1 block">Status</label><select value={form.status} onChange={e => update('status', e.target.value)} className="w-full rounded-lg border border-border-primary bg-surface-card px-3 py-2.5 text-sm"><option value="in-progress">In Progress</option><option value="live">Live</option><option value="archived">Archived</option></select></div>
            <div className="flex items-center gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={form.isPublished} onChange={e => update('isPublished', e.target.checked)} className="accent-blue-500" /><span className="text-sm text-text-secondary">Published</span></label>
            </div>
          </CardContent>
        </Card>
