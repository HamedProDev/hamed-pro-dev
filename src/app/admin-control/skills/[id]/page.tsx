...
export default function EditSkillPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/skills/${params.id}`)
      ...
      .then(d => {
        if (d.success) {
          const s = d.data
          setForm({ name: s.name, category: s.category, proficiency: s.proficiency, color: s.color || '#8b5cf6', order: s.order_index || 0, featured: s.is_published || false })
        }
        setLoading(false)
      })
  ...
  const handleDelete = async () => {
    ...
    router.push('/admin-control/skills')
  }
