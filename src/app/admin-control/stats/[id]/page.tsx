...
export default function EditStatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    const res = await fetch(`/api/stats/${params.id}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, value: String(form.value) }) })
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Button asChild variant="ghost" size="icon"><Link href="/admin-control/stats"><ArrowLeft className="h-4 w-4" /></Link></Button>
      ...
      <form onSubmit={handleSubmit} className="max-w-xl space-y-6">
        <div>
          ...
          <input id="stat-label" name="label" type="text" required value={form.label} onChange={e => setForm({ ...form, label: e.target.value })} className={inputClass} />
        </div>
        <div>
        ...
        <div>
          ...
          <input id="stat-suffix" name="suffix" type="text" value={form.suffix} onChange={e => setForm({ ...form, suffix: e.target.value })} className={inputClass} />
        </div>
        <div>
