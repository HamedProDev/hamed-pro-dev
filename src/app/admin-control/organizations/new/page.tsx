...
export default function NewOrganizationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) router.push('/admin-control/organizations')
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/admin-control/organizations" className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-text-primary mb-4"><ArrowLeft className="h-4 w-4" /> Back to Organizations</Link>
      <h1 className="text-3xl font-bold mb-6">New Organization</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader><CardTitle>Organization Details</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div><label className="text-sm font-medium mb-1 block">Name *</label><Input required value={form.name} onChange={e => update('name', e.target.value)} placeholder="Organization name" /></div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="text-sm font-medium mb-1 block">Team Size</label><Input value={form.team} onChange={e => update('team', e.target.value)} placeholder="10-20" /></div>
            </div>
            <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="text-sm font-medium mb-1 block">Tech Stack (comma separated)</label><Input value={form.tech} onChange={e => update('tech', e.target.value)} placeholder="React, Node.js, AWS" /></div>
            <div><label className="text-sm font-medium mb-1 block">Website</label><Input value={form.website} onChange={e => update('website', e.target.value)} placeholder="https://..." /></div>
