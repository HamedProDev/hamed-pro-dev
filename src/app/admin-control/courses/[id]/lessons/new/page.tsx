...
export default function NewLessonPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) router.push(`/admin-control/courses/${courseId}/lessons`)
  ...
  return (
    <div className="max-w-3xl">
      <Link href={`/admin-control/courses/${courseId}/lessons`} className="inline-flex items-center gap-1 text-sm text-text-muted hover:text-text-primary mb-4"><ArrowLeft className="h-4 w-4" /> Back to Lessons</Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div><label htmlFor="lesson-title" className="text-sm font-medium mb-1 block">Title *</label><Input id="lesson-title" name="title" required value={form.title} onChange={e => update('title', e.target.value)} placeholder="Lesson Title" /></div>
            ...
            <div><label htmlFor="lesson-content" className="text-sm font-medium mb-1 block">Content (Markdown supported)</label><Textarea id="lesson-content" name="content" rows={8} value={form.content} onChange={e => update('content', e.target.value)} placeholder="Write your lesson content here... (supports **bold**, *italic*, lists, code blocks)" /></div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label htmlFor="lesson-videoDuration" className="text-sm font-medium mb-1 block">Video Duration (minutes)</label><Input id="lesson-videoDuration" name="videoDuration" type="number" min="0" value={form.videoDuration} onChange={e => update('videoDuration', e.target.value)} placeholder="e.g. 15" /></div>
            </div>
