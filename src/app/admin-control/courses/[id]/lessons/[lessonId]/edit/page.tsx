...
export default function EditLessonPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/courses/${courseId}/lessons/${lessonId}`).then(r => r.json()),
      ...
      if (lessonData.success && lessonData.data) {
        ...
        setForm({
          ...
          videoDuration: l.videoDuration ? String(l.videoDuration) : '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else setError(data.error || 'Failed to update lesson')
          ...
          <CardContent className="space-y-4">
            ...
            {form.quiz.map((q, qIdx) => (
              <div key={qIdx} className="p-4 rounded-lg bg-surface-secondary border border-border-primary">
                ...
                <div className="mt-3">
                  <label className="text-xs text-text-muted mb-1 block">Explanation</label>
