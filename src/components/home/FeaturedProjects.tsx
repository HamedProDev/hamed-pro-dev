...
const featured = [
  {
    title: 'FarmConnect',
    description: 'Digital marketplace connecting Rwandan farmers directly with buyers.',
    techStack: ['Next.js', 'TypeScript', 'MongoDB'],
    gradient: 'from-blue-600/30 via-purple-600/20 to-dark-800',
    demoUrl: '#',
    sourceUrl: '#',
  },
  {
    title: 'Kwanda EMS',
    description: 'Enterprise management system for Kwanda Facility operations.',
    techStack: ['Next.js', 'Express', 'PostgreSQL'],
    gradient: 'from-indigo-600/30 via-blue-600/20 to-dark-800',
    demoUrl: '#',
    sourceUrl: '#',
  },
  {
    title: 'AI Health Assistant',
    description: 'ML-powered health screening tool for rural communities.',
    techStack: ['Python', 'TensorFlow', 'React Native'],
    gradient: 'from-green-600/30 via-emerald-600/20 to-dark-800',
    demoUrl: '#',
    sourceUrl: '#',
  },
]
          ...
          {featured.map((p, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
              <Card className="h-full card-hover group overflow-hidden">
                <div className={cn('h-52 rounded-t-xl bg-gradient-to-br relative', p.gradient)}>
                  <Badge className="absolute top-4 left-4 bg-green-500 text-white border-0 text-xs">Featured</Badge>
                  {/* Simulated UI elements in preview */}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex items-center gap-4 pt-4 border-t border-white/5">
                    <a href={p.demoUrl} target="_blank" className="flex items-center gap-1.5 text-sm text-text-secondary hover:text-brand-primary transition-colors">
                      <ExternalLink className="h-3.5 w-3.5" /> Live Demo
                    </a>
                    <a href={p.sourceUrl} target="_blank" className="flex items-center gap-1.5 text-sm text-text-secondary hover:text-brand-primary transition-colors">
