...
const categoryConfig: Record<string, { color: string; gradient: string; icon: string }> = {
  Backend: { color: 'bg-violet-500 text-white', gradient: 'from-violet-600/40 via-violet-800/30 to-surface-secondary', icon: 'N' },
  Frontend: { color: 'bg-teal-500 text-white', gradient: 'from-teal-600/40 via-teal-800/30 to-surface-secondary', icon: 'F' },
          ...
          {posts.map((post, i) => (
            ...
            >
              <Link href={`/blog/${post.slug}`}>
                <Card className="h-full group overflow-hidden border-border-primary hover:border-violet-500/20 transition-all duration-300">
                  <div className={cn('h-48 rounded-t-xl bg-gradient-to-br relative flex items-center justify-center overflow-hidden', categoryConfig[post.category]?.gradient || 'from-violet-600/40 via-violet-800/30 to-surface-secondary')}>
                    <motion.span
                    ...
                    <Badge className={cn('absolute top-4 left-4 border-0 text-xs z-10', categoryConfig[post.category]?.color || 'bg-violet-500 text-white')}>
                      {post.category}
                    </Badge>
                  </div>
                  <CardContent className="p-6">
                    <h3 className="text-lg font-semibold mb-2 group-hover:text-violet-400 transition-colors duration-300 line-clamp-2">{post.title}</h3>
